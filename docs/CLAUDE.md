# DDLR AI DRAFTING PROJECT — CLAUDE CODE INSTRUCTIONS

> ⚠ **SUPERSEDED PRICING NOTICE — 2026-04-27**
> Pricing tables in this document (Pack A/B/C/D) are outdated.
...
3. **NEVER modify registry** (HKLM or HKCU) without FIRST creating a System Restore point: `Checkpoint-Computer -Description "Before Claude changes"`.
...
9. **ALWAYS confirm before ANY destructive system action** — even if CLAUDE.md says "action first, explain after", system-level changes are the exception.
...
## TEAM ARCHITECTURE — Cowork + Claude Code = ONE Team
...
| **Claude Code (you)** | The HANDS | Coding, testing, executing, building, deploying |
...
2. Fix the issue
3. Add a NEW CODE RULE to CLAUDE.md to prevent the same mistake
...
2. Add a new RULE to CLAUDE.md CODE RULES section
3. Log the CLAUDE.md change in PROMPT_CHANGELOG.md
...
|-------|-------|------|--------|
| Claude Opus (Projects) | 98 | ₹40 | Champion but expensive |
| Claude Sonnet 4.6 | 96 | ₹12 | Premium option |
...
- AI premium: Claude Sonnet 4.6 via OpenRouter (₹12, 96/100)
- Database: Neon PostgreSQL (planned)
- Automation: n8n (planned)
- Agent framework: Claude Agent SDK (planned Phase 1B)
