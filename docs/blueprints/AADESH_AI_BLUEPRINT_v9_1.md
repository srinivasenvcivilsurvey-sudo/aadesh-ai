# AADESH AI — BLUEPRINT v9.1
# Date: April 6, 2026 | Status: ACTIVE
# This is the ONLY blueprint. v8.0-v8.2 and v9.0 are archived.
# v9.1 patches: 8 findings from triple cross-validation (Perplexity + Gemini + GPT)
...
Claude Code MUST structure the Anthropic API call in THIS exact order:
...
| 15 | Test with 3 real Banu cases end-to-end | P0 | 1 day | TO DO |
| 16 | Give Banu login, monitor first 5 orders | P0 | 2 days | TO DO |
| **TOTAL Phase 0** | | | **~12 days** | |

### Phase 1 (after Banu validates — April-May 2026):

| # | Task | Priority | Time |
|---|------|----------|------|
| 17 | Onboarding flow (office type, district, case types) | P1 | 1 day |
| 18 | Training page (upload references, status bar) | P1 | 1 day |
| 19 | Smart reference selection (top 5-8 by case type match) | P1 | 1 day |
| 20 | Self-audit loop (4 guardrails) | P1 | 0.5 day |
| 21 | User rating before download | P1 | 0.5 day |
| 22 | Admin dashboard for Srinivas (/admin) | P1 | 1 day |
| 23 | Referral system (invite link + 5 free credits) | P1 | 0.5 day |
| 24 | DPIIT Startup India registration | P1 | 1 week (govt process) |
| 25 | **API queue system** — Supabase job_queue table. Prevents rate limit crashes when 5+ officers generate at once. | P1 | 1 day |
| 26 | **Client-side image compression** — Browser compresses phone photos to <2MB before upload. | P1 | 0.5 day |
| 27 | **Upgrade VPS to 4GB RAM** — 1GB crashes at 3-4 concurrent users. $24/mo (~Rs 2,000). | P1 | 1 hour |
| 28 | **Request Anthropic API tier upgrade** — Apply for Tier 2/3 before reaching 10 users. | P1 | 1 day |
| 29 | **Test Gemini 2.5 Flash-Lite on 1 Banu case** — 30x cheaper than Claude ($0.10/$0.40 vs $3/$15). If 80+/100, use for simple cases. | P1 | 1 day |
| 30 | **Bhashini ULCA STT integration** — FREE Kannada voice-to-text from govt Digital India program. Available NOW. Older officers resist typing. | P1 | 0.5 day |
| 31 | **Abstract database layer** — thin wrapper so Supabase can be swapped for plain Postgres in hours. Highest-ROI defensive move. | P1 | 1 day |

### Phase 2 (10+ users — June-July 2026):

| # | Task | Priority |
|---|------|----------|
| 32 | Bible generation from reference orders (style optimization) | P2 |
| 33 | Edit tracking and learning loop | P2 |
| 34 | Global knowledge pool (demo mode) | P2 |
| 35 | Transfer mode (officer moves to new office) | P2 |
| 36 | Multi-office expansion (BBMP, BDA, Sub-Registrar) | P2 |
| 37 | Supabase self-hosting on VPS (Docker) — eliminates India block risk entirely | P2 |
| 38 | WhatsApp Business integration — share DOCX via WhatsApp, primary officer channel | P2 |
...
| Sarvam Chanakya (govt AI platform) | Sarvam AI | **COMPETITOR** — they have govt relationships + Kannada models. If they add document drafting, they reach every officer through institutional channels we cannot access. Watch closely. |
...
| **D-9.22** | **Gemini 2.5 Flash-Lite = test in Phase 1** | 30x cheaper than Claude. If 80+/100 quality, use for simple cases. | Apr 6, 2026 |
...
| **v9.1** | **Apr 6** | **CROSS-VALIDATION PATCH. 8 findings from 3 independent red-team reviews (Perplexity DR + Gemini + GPT). Added: Sentry monitoring, UptimeRobot, per-user rate limits, VPS security hardening, prompt_version tracking, regeneration cost buffer (25%), Anthropic DPA, Sarvam reclassified as competitor, Gemini 2.5 Flash-Lite evaluation (Phase 1), Bhashini STT moved to Phase 1, DB abstraction layer, WhatsApp integration (Phase 2). 26 decisions total. 38 build tasks (16 P0 + 15 P1 + 7 P2).** |
...
# END OF BLUEPRINT v9.1
# This is the ONLY active blueprint.
# All previous versions (v6.7 through v9.0) are archived for reference only.
# Cross-validated by: Claude (CTO) + Perplexity Deep Research + Gemini + GPT
# Total reviews: 5 rounds (Gemini x2, Perplexity x1, GPT x1, Claude self-audit x1)
